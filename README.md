...
<!-- test-stats:begin -->
合计 **281 项 Rust 测试 + 409 项前端测试 + 56 项协议契约断言**——
...
> [`docs/协议勘误与修正.md`](docs/协议勘误与修正.md) §3.25）。修复后首次真跑：
> **verify 与 web 两个 job 全绿**，rust job 的单元测试通过，
> 仍有 3 个集成测试在 Linux 上失败（§3.27，已记录待修）。
> 本机（macOS）全部测试通过——这些是**平台差异**，不是本地可复现的问题。
