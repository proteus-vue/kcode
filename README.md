...
> 而一次都没执行过。2026-09-24 CI #70 起三个 job 全绿——这句至此才真正成立。）*
...
> 在本仓库工作（人或 AI）前请先读 [`AGENTS.md`](AGENTS.md)：
> 收尾指令的语义、效率上的硬性要求，以及「未验证必须标注」等记录规矩。

> **CI 状态：三个 job 全绿**（2026-09-24，CI #70 起）——
> `verify` / `rust` / `web` 全部通过，覆盖协议漂移、协议事实清单、codex 哈希、
> 端到端契约、Rust 全量测试（单元 + 集成 + clippy）、测试数字漂移、
> 出站连接、Tauri capability、前端单测、类型检查与构建。
>
> **这是本项目 CI 第一次真正通过。** 此前它有两个层面的问题：
>
> **一、它长期没有真正执行过。** 因一处 `if:` 表达式失效，
> 从项目第一次提交起 18 次推送全部空跑（§3.25）——「由 CI 持续验证」
> 这句承诺曾经是空的。
>
> **二、能执行之后，失败信息一度读不到。** 三条读取通道里只有 check-run
> 注解真的通（有 23 条配额），job summary 对未登录访问者不渲染、
> artifact 需登录（§3.31）。而 cargo 的错误行带 ANSI 颜色码，
> 使所有按 `^error` 写的判据落空——于是「编译期错误」被连续多轮误判成
> 「资源不足」，方向是错的（§3.32）。先让失败可见，才谈得上修。
>
> 沿途排掉的三个真实根因：
>
> | 症状 | 根因 | 影响面 |
> |---|---|---|
> | `单元测试 · kcode-desktop` 编译失败 | `bundle.resources` 声明的 `binaries/` 打包时才生成、刻意不入库，而 tauri 的 build script **在编译期**要求它存在（§3.32） | **任何全新 clone 都编译不过**（含新贡献者） |
> | `静态检查`（clippy）红 | CI 用滚动的 `stable`（1.98.1），1.98 新增的 `manual_slice_fill` 命中一行旧代码；本机停在 1.95（§3.33） | 工具链漂移，与代码变更无关 |
> | `测试数字漂移检查`红 | 契约脚本的 `CODEX_HOME` 建在 `/tmp`，而 codex **拒绝在临时目录创建沙箱助手**（防提权），导致 `command/exec` 的 tty 模式无法执行（§3.34） | 测试夹具缺陷；产品用 `app_data_dir()`，不受影响 |
>
> 三条经验已固化进 CI 本身：诊断入口**无条件剥 ANSI**、
> **rustc/clippy 版本进注解**（否则「本地绿、CI 红」无法对表）、
> 每个可能失败的步骤都有**可读的失败摘要**（且不会在没资格跑的时候跑）。
