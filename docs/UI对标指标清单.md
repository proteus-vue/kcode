...
| SC-09 | 移动端模拟器展示 | MiMo 侧栏「模拟器」 | **已落地**：Android 走 `adb`（实时截图 + 点击/滑动/返回/主屏，单帧实测 350ms）；iOS 需完整 Xcode。可用性由后端探测（不可用不给空入口），工具缺失时如实说明缺什么与安装指引 | ✅ | P2 |
