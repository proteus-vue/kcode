...
| SC-09 | 移动端模拟器展示 | MiMo 侧栏「模拟器」 | **已落地**：Android 走 `adb`（实时截图 + **点击/滑动**/返回/主屏，单帧实测 350ms）；iOS 需完整 Xcode。可用性由后端探测（不可用不给空入口），工具缺失时如实说明缺什么与安装指引。**触控回传**：`pointerdown/move/up` → `simulatorGesture` 判定点击/滑动（阈值 6px 容差、时长按实际拖动夹紧到 50–1000ms）→ `adb shell input tap/swipe`；输入后**立即补一帧**（不等 600ms 轮询），点击有落点标记、拖动有轨迹线。21 项测试（12 项判定 + 9 项接线） | ✅ | P2 |
...
- **模拟器场景**：Android 真画面（adb 实时截图 + 输入），iOS 如实提示缺 Xcode；可用性由后端探测。
...
5. **模拟器未覆盖**：iOS 侧（需完整 Xcode）、滑动**惯性**（滑动本身已做，缺的是抬手后的惯性滚动）、多指手势、音频
