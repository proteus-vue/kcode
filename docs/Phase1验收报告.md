...
|---|---|
| 反向 DNS | 无 PTR 记录 |
| 比对已知域名 | `api.openai.com` → `2a03:2880:f11f:...`；`chatgpt.com` → `2a03:2880:f112:...`。**同属一个 /32 段**，判定为 OpenAI 的 CDN 段（Meta 托管） |
...
但它确实构成一次**用户未显式配置的出站尝试**，因此：

1. **不宣称「零出站」**——`SECURITY.md` 已新增专节如实披露，标注为上游行为、触发条件、可否关闭。
2. **让事实可见**：`verify-no-egress.sh` 把它显式列入白名单并**单独计数报告**，
   而非静默忽略。任何**第三类**（既非本地 provider 也非该 CDN 段）的连接都会导致脚本失败。
3. **提供规避路径**：文档说明可在防火墙/hosts 层阻断该段。
